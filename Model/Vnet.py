...
class LinearModel(nn.Module):
    def __init__(self):
        ...
        self.linear1 = nn.Linear(96, 192)
        self.linear2 = nn.Linear(192, 216)
    ...
    def forward(self, x):
        ...
        if len(x.shape) == 1:
            x = torch.reshape(x, (6, 6, 6))
        else:
            x = torch.reshape(x, (x.shape[0], 1, 6, 6, 6))
            x = x.repeat(1, 256, 1, 1, 1)
...
class VNet(nn.Module):
    ...
    def decoder(self, features, tokens):
        # 4, 16, 112, 112, 80
        x1 = features[0]
        ...
        # merging tokens
        token_features = self.linear(tokens)
        x5 += token_features
    ...
    def forward(self, input, tokens, turnoff_drop=False):
        if turnoff_drop:
            has_dropout = self.has_dropout
            self.has_dropout = False

        features = self.encoder(input)
        out = self.decoder(features, tokens)
        if turnoff_drop:
            self.has_dropout = has_dropout
        return out
