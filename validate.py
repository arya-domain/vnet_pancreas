...
def test_all_case(net, val_set, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4,
                  post_process=False, save_path=None):
    total_metric = 0.0
...
def test_single_case(net, image, tokens, stride_xy, stride_z, patch_size, num_classes=1,
                     post_process=False):

    image = image.squeeze()
    w, h, d = image.shape

    # if the size of data is less than patch_size, then padding it
    add_pad = False
    if w < patch_size[0]:
...
def test_calculate_metric(ckpt_path, path_=None, post=False):
    ...
    avg_metric = test_all_case(net, val_dataset, num_classes=2,
                               ...
                               post_process=post, save_path=path_)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--vis_path", default="visual", type=str,
                        help="your visualisation path")
    ...
    print("validate {} for Pancreas dataset ...".format(str(ckpt)))
    metric = test_calculate_metric(os.path.join(default_path, ckpt), path_=cmd_line.visual,
                                   post=cmd_line.post)
